...
from llmcompressor.modifiers.awq_s.mappings import (
    AWQMapping,
    ResolvedMapping,
...
__all__ = ["AWQSModifier"]


class AWQSModifier(Modifier, QuantizationMixin):
    """
    Custom AWQ Modifier (awq_s) - Based on Activation-Weighted Quantization algorithm.
    ...
    def _simple_column_check(
        ...
        for i in range(1, len(scales)):
            ...
            if ratio_change < 0.85:
                # 太小了，要讓 curr_ratio 變大 → 減小 scales[i]
                target_ratio = prev_ratio * 0.85  # 設為下限
                adjusted_scales[i] = x_mean[i] / (target_ratio + 1e-8)
                
            elif ratio_change > 1.15:
                # 太大了，要讓 curr_ratio 變小 → 增大 scales[i]
                target_ratio = prev_ratio * 1.15  # 設為上限
                adjusted_scales[i] = x_mean[i] / (target_ratio + 1e-8)
